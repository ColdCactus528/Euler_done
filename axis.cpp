...
void Axis::test() {
    ...
    QQuaternion qRoll  = QQuaternion::fromAxisAndAngle(QVector3D(1,0,0), ang0);
    QQuaternion qPitch = QQuaternion::fromAxisAndAngle(QVector3D(0,1,0), ang1);
    QQuaternion qYaw   = QQuaternion::fromAxisAndAngle(QVector3D(0,0,1), ang2);

    QQuaternion q = qRoll * qPitch * qYaw;
    // QVector3D vec = quatToEulTest("Z-Y-Z", q, flag0);
    QVector3D vec = quatToAirTest("X-Y-Z", q, flag0);
...
QVector3D Axis::quatToAirTest(const QString& order, QQuaternion q, int flag) {
    ...
    if (order == "X-Y-Z") {
        ...
        if (cy > 16 * FLT_EPSILON) {
            ...
            // else if (flag == 2)
            //     beta_rad = atan2(-M[2][0], cy);
            // else if (flag == 3)
            //     beta_rad = atan2(cy, -M[2][0]);
